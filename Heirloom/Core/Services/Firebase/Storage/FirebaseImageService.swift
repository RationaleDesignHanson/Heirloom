...
class FirebaseImageService: FirebaseImageServiceProtocol {
    ...
    func uploadImage(for recipe: Recipe) async throws -> String? {
        ...
        guard let image = await recipe.loadImage() else {
            logger.log("No local image found for recipe upload: \(recipe.title)", category: .storage, level: .warning, metadata: nil)
            return nil
        }

        // Compress image (max 1MB for efficient upload)
        guard let imageData = await compressImage(image, maxBytes: 1_000_000) else {
            logger.log("Failed to compress image for upload: \(recipe.title)", category: .storage, level: .warning, metadata: nil)
            return nil
        }
        ...
        logger.log("Uploading recipe image to Firebase Storage: \(recipe.title)", category: .storage, level: .info, metadata: nil)
        ...
        logger.log("Recipe image uploaded successfully", category: .storage, level: .info, metadata: nil)
    ...
    func downloadImage(for recipe: Recipe) async throws {
        ...
        if let imageFileName = recipe.imageFileName,
           await recipe.loadImage() != nil {
            logger.log("Image already cached locally: \(imageFileName)", category: .storage, level: .debug, metadata: nil)
            return
        }

        logger.log("Downloading recipe image from Firebase Storage: \(recipe.title)", category: .storage, level: .info, metadata: nil)
        ...
        guard let image = UIImage(data: imageData) else {
            logger.log("Failed to decode image data from Firebase Storage", category: .storage, level: .warning, metadata: nil)
        ...
        logger.log("Recipe image downloaded and cached: \(recipe.imageFileName ?? "unknown")", category: .storage, level: .info, metadata: nil)
    }
    ...
    func deleteImage(for recipeId: UUID) async throws {
        ...
        do {
            ...
            logger.log("Recipe image deleted successfully", category: .storage, level: .info, metadata: nil)
        } catch {
            // Ignore "not found" errors (image may not exist)
            if (error as NSError).code == StorageErrorCode.objectNotFound.rawValue {
                logger.log("Image not found in Firebase Storage (already deleted)", category: .storage, level: .debug, metadata: nil)
            } else {
                throw error
            }
        }
    ...
    func deleteImage(at url: String) async throws {
        ...
        logger.log("Deleting recipe image from Firebase Storage by URL", category: .storage, level: .info, metadata: nil)
