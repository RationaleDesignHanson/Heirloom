...
final class Recipe {
    ...
    // MARK: - Social (Phase 2)
    var sharedBy: String? // Sharer's display name
    var sharedByUserId: String? // Sharer's Firebase user ID (for querying their recipes)
    var sharedFromRecipeId: String? // Original recipe UUID (lowercase) from sharer's collection
    var sharedDate: Date?
    ...
    /// Ordered list of user IDs in the heritage chain (root → ... → current)
    /// Used for offline display and backup/restore of lineage
    /// Stored as JSON to avoid SwiftData array materialization issues
    var heritageChainJSON: String?

    /// Computed property for safe access to heritage chain
    var heritageChain: [String]? {
        get {
            guard let json = heritageChainJSON,
        ...
        set {
            if let newValue = newValue,
               ...
               let json = String(data: data, encoding: .utf8) {
                heritageChainJSON = json
            } else {
                heritageChainJSON = nil
            }
        }
    }

    /// Display names corresponding to heritageChain user IDs
    /// Enables offline display of "Grandma Rose → Mom → You"
    /// Stored as JSON to avoid SwiftData array materialization issues
    var heritageChainNamesJSON: String?

    /// Computed property for safe access to heritage chain names
    var heritageChainNames: [String]? {
        get {
            guard let json = heritageChainNamesJSON,
                  let data = json.data(using: .utf8),
                  let array = try? JSONDecoder().decode([String].self, from: data) else {
                return nil
            }
        ...
        set {
            if let newValue = newValue,
               let data = try? JSONEncoder().encode(newValue),
               let json = String(data: data, encoding: .utf8) {
                heritageChainNamesJSON = json
            } else {
                heritageChainNamesJSON = nil
            }
        }
    }

    /// When full lineage tree was last cached for offline viewing
    /// nil means not cached; Netflix-style explicit download
    var lineageCachedAt: Date?

    /// Cached lineage tree data as JSON for offline viewing
    /// Contains full tree structure fetched from Firebase
    var cachedLineageJSON: String?

    /// Whether this recipe was received from a demo user (onboarding)
    /// Demo recipes cannot be shared - they're for learning only
    var isDemoRecipe: Bool = false

    // MARK: - AI Generation Tracking
    /// Whether this recipe was generated by AI from minimal input
    var aiGenerated: Bool = false

    /// Whether this recipe was created via voice dictation
    var voiceDictated: Bool = false
