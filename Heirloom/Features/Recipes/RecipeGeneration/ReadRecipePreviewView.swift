...
/// Preview screen for voice-dictated recipes with public sharing option
struct ReadRecipePreviewView: View {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    Toggle(isOn: $isPublic) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Share to Discover")
                                .font(HeirloomFonts.body)
                                .fontWeight(.medium)

                            Text("Make this recipe public for others to discover")
                                .font(HeirloomFonts.caption1)
                                .foregroundStyle(.secondary)
                        }
                    }
    ...
    private func saveRecipe() {
        ...
        // Recipe is already inserted in context by extractor
        // Just save the context and call onSave
        do {
            try modelContext.save()
            onSave()
        } catch {
            Log.error("Failed to save voice-dictated recipe", category: .general, metadata: [
