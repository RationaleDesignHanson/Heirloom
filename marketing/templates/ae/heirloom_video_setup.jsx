 ...
 * video production project with all 13 compositions, placeholder layers,
...
(function() {
    ...
    var AD_CONFIGS = [
        ...
        {
            name: "V10_FREE_DAILY_CREDITS",
            duration: 15,
            hookType: "Save action",
            captures: ["CAP_03", "CAP_02", "CAP_21"],
            hookText: "Free daily credits to save any recipe",
            ctaText: "Download Heirloom"
        },
        {
    ...
    function createAppStoreComp() {
        ...
        // Layer stack (added bottom to top, so add in reverse order)

        // [27-30s] Block E: CTA
        var ctaLayer = comp.layers.add(blockComps.E);
        ctaLayer.startTime = 27;
        ctaLayer.outPoint = 30;
        ctaLayer.name = "BLOCK_E_CTA";

        // [22-27s] CAP_11 (saved toast) - Note: Using a placeholder since CAP_11 wasn't in original spec
        var cap11 = addPlaceholder(comp, "CAP_11", 22, 5, w, h);
        addTransitionMarker(cap11, 22, "Saved Toast");

        // [19-22s] CAP_07B (collections)
        var cap07b = addPlaceholder(comp, "CAP_07B", 19, 3, w, h);
        addTransitionMarker(cap07b, 19, "Collections");

        // [16-19s] CAP_13 (attribution)
        var cap13 = addPlaceholder(comp, "CAP_13", 16, 3, w, h);
        addTransitionMarker(cap13, 16, "Attribution");

        // [12-16s] CAP_05 (privacy pills)
        var cap05 = addPlaceholder(comp, "CAP_05", 12, 4, w, h);
        addTransitionMarker(cap05, 12, "Privacy Pills");

        // [6-12s] Magic sequence: CAP_03B → CAP_12 → CAP_15
        var cap15 = addPlaceholder(comp, "CAP_15", 10, 2, w, h);
        var cap12 = addPlaceholder(comp, "CAP_12", 8, 2, w, h);
        var cap03b = addPlaceholder(comp, "CAP_03B", 6, 2, w, h);
        addTransitionMarker(cap03b, 6, "Magic Sequence Start");

        // [2.5-6s] CAP_03 (share sheet)
        var cap03 = addPlaceholder(comp, "CAP_03", 2.5, 3.5, w, h);
        addTransitionMarker(cap03, 2.5, "Share Sheet");

        // [0-2.5s] Block A: Hook
        var hookLayer = comp.layers.add(blockComps.A);
        hookLayer.startTime = 0;
        hookLayer.outPoint = 2.5;
        hookLayer.name = "BLOCK_A_HOOK";

        // Text overlays
        addTextLayer(comp, "One tap. Real recipe.", 2.5, 3.5, h * 0.1);
        addTextLayer(comp, "The magic happens here", 6, 6, h * 0.1);
        addTextLayer(comp, "Private by default", 12, 4, h * 0.1);
        addTextLayer(comp, "Credit creators", 16, 3, h * 0.1);
        addTextLayer(comp, "Organize beautifully", 19, 3, h * 0.1);
        addTextLayer(comp, "Saved!", 22, 5, h * 0.1);
    ...
    function main() {
        ...
        try {
            ...
            alert(
                ...
                "• 17 capture placeholders (CAP_XX)\n" +
                "• 5 block pre-compositions\n" +
