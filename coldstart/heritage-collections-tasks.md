...
First, add this context to your project's `CLAUDE.md` file (create one if it doesn't exist). This gives Claude Code persistent context about the feature:

```markdown
## Current Feature: Cold Start Solution & Collections UI

### Reference Documents
- `docs/heritage-collections-spec.md` - Complete design specifications
- `docs/heritage-collections-tasks.md` - Implementation task list

### Goal
Replace the "Add" bottom tab with "Collections" and seed the app with 100 public domain heritage recipes organized into 4 founding collections. This solves the cold start problem by showing users what preserved recipes look like.

### The Four Founding Collections
1. **The Presidential Pantry** — White House recipes from Washington to Obama (source: National Archives, Library of Congress)
2. **The Literary Kitchen** — Recipes from authors' diaries/letters (Dickinson, Austen, Twain, Monet)
3. **The Ancient Table** — Roman (Apicius), Medieval (Forme of Cury), ancient world recipes
4. **The American Foundation** — First African American cookbooks (Abby Fisher, Malinda Russell), Rosa Parks, suffrage cookbooks

### Key Concepts
- **Heritage recipes**: Read-only seed content with dual-view (original archaic text + modernized instructions)
- **Founding profiles**: Simulated curator accounts for each collection (not real users)
- **Bulk organization**: Selection mode in collections for move/copy/remove/tag operations
- **Add flow relocation**: FAB replaces the bottom tab for adding recipes
- **Heritage imagery**: Each recipe has both archival imagery (when available) and AI-generated hero images

### Data Sources (all public domain)
- Pre-1930 published cookbooks
- US Government publications (White House, National Archives, USDA)
- Ancient texts (Apicius, Forme of Cury)

### Image Sources
- **Public domain archives**: Library of Congress, Wikimedia Commons, NYPL Digital Collections, Smithsonian Open Access, Internet Archive
- **AI generation**: Google Imagen API for recipes without archival imagery
- **Style**: Warm, nostalgic, painterly/vintage aesthetic — NOT modern food photography
- See `docs/heritage-collections-spec.md#image-strategy` for full details

### API Keys Available
- `ANTHROPIC_API_KEY` — for Claude API (image analysis, prompt generation)
- `GOOGLE_API_KEY` — for Imagen API (image generation)
```

---

## Step 2: Sequential Task Prompts
